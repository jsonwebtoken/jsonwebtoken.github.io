...
You will find the generated files in the `dist` directory. For the website, you can run a server at its root. For example: `brew install http-server && http-server dist/website`. The default `grunt` task runs a server
